// module/ATChat.js — v13.0.5.4
const MODULE_ID = "about-time-v13";
const AT = () => (game.abouttime ?? game.Gametime);
if (!globalThis.__AT_CHAT_REGISTERED__) {
  globalThis.__AT_CHAT_REGISTERED__ = true;
  ...
  Hooks.on("chatMessage", (chatLog, message) => {
    ...
    try {
      switch (sub) {
        ...
        case "advance": {
          ...
          const seconds = parseDuration(durStr);
          if (!seconds || seconds <= 0) { reply("Invalid duration."); break; }
          game.time.advance(seconds).then(() => {
    ...
    return false; // prevent the literal line from posting
  });
}
